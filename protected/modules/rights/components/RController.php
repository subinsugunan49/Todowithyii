...
class RController extends CController
{
	...
	*/
	...
	{
		 if(Yii::app()->user->checkAccess('Employee_Timeoff_Request'))
			 $_SESSION['timeoff']=1;
			else
			$_SESSION['timeoff']=0;
		...
		{
			$StationMasterId=array();
		    $users = Yii::app()->db->createCommand()
			->select('u.regionID,u.user_type')
			->from('users u')
			->where('u.id =:id', array(':id'=>$_SESSION['user_id']))			
			->queryRow();
			
		if(isset($users['regionID']))
		$_SESSION['regionID']=$users['regionID'];
